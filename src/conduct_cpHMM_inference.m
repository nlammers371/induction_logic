...
%-------------------------------System Vars-------------------------------%
w = ceil((1.302 +  1.085/2)/1.5); % Memory 
K = 2; % State to use for inference
project = '20200807';  %identifier pointing to desired data set
groups_to_run = 1;
...
% Load data for inference into struct named: trace_structure
load([datapath dataname]);
alpha = trace_structure(1).alpha_frac*w; % Rise Time for MS2 Loops in time steps
Tres = trace_structure(1).Tres; % Time Resolution

% get vector of group IDs 
group_id_vec = [trace_structure.setID];
bin_groups = unique(group_id_vec(ismember(group_id_vec,groups_to_run)));
% Set write path (inference results are now written to external directory)
out_dir =  ['../out/' project '/w' num2str(w) '/K' num2str(K) '/raw_cpHMM_output/']; 
mkdir(out_dir);
...
for g = 1:length(bin_groups) % loop through different AP groups
    ...
    for b = 1:n_bootstrap % iterate through bootstrap replicates
        ...
        inference_set = trace_structure(trace_filter); % find eligible traces

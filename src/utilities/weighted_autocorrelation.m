...
function [wt_autocorr, a_boot_errors, wt_d2, d2_boot_errors] = ...
  weighted_autocorrelation(traces, lags, bootstrap,n_boots,trace_weights)
    ...
    t_filter = true(1,size(traces,2));%mean(traces>0)>0.1;
    ...
    d2_samples = zeros(lags-1,n_boots);
    for b = 1:n_boots
        ...
        for col = 1:size(trace_sample,2)            
            ...
            trace_active = trace(find(trace>0,1):find(trace>0,1,'last'));
        ...
        d2_samples(:,b) = diff(samples(:,b),2);
    end
    wt_autocorr = mean(samples,2);
    wt_d2 = mean(d2_samples,2);
    a_boot_errors = std(samples,[],2);
    d2_boot_errors = std(d2_samples,[],2);